...
export async function matchItemPhoto({ env, payload, requestUrl }) {
  ...
  try {
    const rerankResult = await rerankCandidates(apiKey, model, imageDataUrl, shortlistWithImages);

    if (shouldSuppressMatch(rerankResult)) {
      return buildNoMatchResponse(index, model, startedAt, queryProfile, shortlistWithImages, rerankResult);
    }
...
function buildNoMatchResponse(index, model, startedAt, queryProfile, shortlist, rerankResult) {
